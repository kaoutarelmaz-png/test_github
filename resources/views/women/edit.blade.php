@extends('layouts.app')
@section('title','Edit Product Women')
@section('content')
            ...
            <div class="card" style="border: 0.5px solid gray">
                <div class="card-header text-black py-3" style="background-color: #004d99; border-radius: 10px 10px 0 0;">
                    <h2 class="text-center mb-0 h5 text-white">
                        <i class="fas fa-edit me-2"></i>Edit Product Women
                    </h2>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('women.update', $edits->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" name="code_article_womans" class="form-control" 
                                       value="{{ $edits->code_article_womans }}" required maxlength="255" style="border: 0.5px solid black;">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="image-preview mt-2" id="imagePreview" style="border: 0.5px solid black;">
                                    <img src="{{ Storage::url('imager/' . $edits->imager) }}" alt="Preview" style="display: block; width:100%; height:100px; object-fit:cover; border-radius:8px;">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mt-2">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="size" class="form-select" required style="border: 0.5px solid black;">
                                    <option value="" disabled>Select size</option>
                                    <option value="XS" {{ $edits->size=='XS'?'selected':'' }}>XS</option>
                                    <option value="S" {{ $edits->size=='S'?'selected':'' }}>S</option>
                                    <option value="M" {{ $edits->size=='M'?'selected':'' }}>M</option>
                                    <option value="L" {{ $edits->size=='L'?'selected':'' }}>L</option>
                                    <option value="XL" {{ $edits->size=='XL'?'selected':'' }}>XL</option>
                                    <option value="XXL" {{ $edits->size=='XXL'?'selected':'' }}>XXL</option>
                                </select>
                            </div>
