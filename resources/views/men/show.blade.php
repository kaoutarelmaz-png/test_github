    ...
    <form action="{{ route('carte.index') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="product-container">
            ...
            <div class="product-details">
                <input type="hidden" name="IDmen" value="{{ $detaillers->id }}">        
                <h1>{{ $detaillers->title }}</h1>
                <p>{{ $detaillers->content }}</p>
                <div class="price">${{ $detaillers->price }}</div>
                <div class="price">{{ $detaillers->size }}</div>
                ...
                <a href="{{ route('men.index') }}"  class="btnAnnuler">Annuler</a>
                </div>
        </div>
<div class="container">
    @foreach($menssse as $mensssesse)
        <a href="{{ route('men.show', $mensssesse->id) }}" style="text-decoration: none;">
            <div class="card">
