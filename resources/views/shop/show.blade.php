...
<div class="navbar">
    ...
    <form action="{{ route('carte.index') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="product-container">
            <!-- Image on the left -->
            <div>
                <img src="{{ Storage::url('imager/').$detaillers->imager }}" width="250px" alt="Product Image" class="product-image">
            </div>
            <!-- Product details on the right -->
            <div class="product-details">
                <input type="hidden" name="IDshop" value="{{ $detaillers->id }}">        
                <h1>{{ $detaillers->title }}</h1>
                <p>{{ $detaillers->content }}</p>
                <div class="price">${{ $detaillers->price }}</div>
                <div class="size">{{ $detaillers->size }}</div>
                ...
                <a href="{{ route('shop.index') }}"  class="btnAnnuler">Annuler</a>
                </div>
        </div>
        <div class="container">
    @foreach($menssse as $mensssesse)
        <a href="{{ route('shop.show', $mensssesse->id) }}" style="text-decoration: none;">
            <div class="card">
